...
const FindDog = () => {
  ...
  const [filters, setFilters]           = useState({
    breed: "Any", age: "Any", size: "Any", gender: "Any",
  });
  ...
  useEffect(() => {
    const fetchDogs = async () => {
      try {
        ...
        const dogs = data.filter(p => p.species === "Dog");
        setAllDogs(dogs);
  ...
  // Get unique breeds from DB data
  const breeds = ["Any", ...new Set(allDogs.map(d => d.breed).filter(Boolean))];
  ...
  const resetFilters = () => {
    setFilters({ breed: "Any", age: "Any", size: "Any", gender: "Any" });
          ...
          {loading ? (
            ...
            <div className="grid grid-cols-3 gap-6">
              {filteredDogs.length > 0 ? filteredDogs.map(dog => (
                <div key={dog._id} className="bg-white rounded-xl shadow-md overflow-hidden hover:shadow-lg transition">
                  ...
                  <div className="p-4">
                    <h3 className="text-lg font-bold text-orange-600">{dog.name}</h3>
                    <p className="text-sm text-gray-600 mb-3">{dog.breed} • {dog.age} • {dog.gender}</p>
                    <button
                ...
                <div className="col-span-3 flex flex-col items-center justify-center py-16 text-center">
                  ...
                  <p className="text-gray-500 text-lg font-medium">No dogs match your filters.</p>
